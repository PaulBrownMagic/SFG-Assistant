...
logtalk_library_path(third_party_libraries, home('Development/MyLogtalkLibs/')).
...
logtalk_library_path(sfg, home('Development/SFG-Assistant/')).
logtalk_library_path(Library, sfg(LibraryPath)) :-
	logtalk::expand_library_path(sfg, SFGLibrariesPath),
	os::directory_files(SFGLibrariesPath, Libraries, [type(directory), dot_files(false)]),
	list::member(Library, Libraries),
	atom_concat(Library, '/', LibraryPath).

logtalk_library_path(storage, sfg(frames_data)).
